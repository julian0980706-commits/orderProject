...
const docRef=doc(db,"forms","southSeaChickenRice")
let ifShow
//loading
...
if(ifShow.show==true){
    ...
    //
    const orderRef=collection(db,"forms","southSeaChickenRice","southSeaChickenRice",)
    ...
    const handleSubmit=async(e)=>{
        ...
        try{
            ...
            //傳完才執行下面
            localStorage.setItem("southSeaChickenRice", num);
            submit_btn.innerText="送出訂單"
            main.style.display="none"
            animation.style.display="flex"
            animation.style.display="flex"
            container.style.display="none"
            setTimeout(()=>{window.location.href="southSeaChickenRice_remember.html"},1200)
        }
        catch(error){
            alert("嗚嗚嗚，錯了")
            location.reload()
