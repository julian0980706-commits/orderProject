...
const docRef=doc(db,"forms","steak")
let ifShow
//loading
...
if(ifShow.show==true){
    ...
    document.getElementById("random").addEventListener("click",async(e)=>{
        ...
        const sauce=["黑胡椒","洋蔥","綜合"]
        document.querySelector('#sauce').value=sauce[Math.floor(Math.random()*3)]
        const sidedish=["玉米濃湯","紅茶"]
        document.querySelector('#sidedish').value=sidedish[Math.floor(Math.random()*2)]
    ...
    const esidedish=document.getElementById("esidedish")
    const error_num=document.getElementById("enum")
    const esauce=document.getElementById("esauce")
    const error_input=document.getElementsByClassName("e")
    const error_deal=()=>{
        for(let i=0 ;i<error_input.length;i++){
            error_input[i].style.display="none"
    ...
    const handleSubmit=async(e)=>{
        ...
        const sidedish=document.querySelector('#sidedish').value
        //確認使用者輸入
        if(!num){
            error_deal()
            error_num.style.display="block"
            error_num.scrollIntoView({behavior:"smooth"})//滾動盪那裏(smoothly)
            return;
        }
        if(!meal){
            error_deal()
        ...
        if(sauce=="醬料"){
            error_deal()
            esauce.style.display="block"
            esauce.scrollIntoView({behavior:"smooth"})//滾動到那裏(smoothly)
            return;
        }
        if(sidedish=="湯or紅茶"){
            error_deal()
            esidedish.style.display="block"
            esidedish.scrollIntoView({behavior:"smooth"})//滾動到那裏(smoothly)
            return;
        }
        //準備傳送(建立物件)
        const total={
        ...
        try{
            ...
            //傳完才執行下面
            localStorage.setItem("steak", num);
            submit_btn.innerText="送出訂單"
            main.style.display="none"
            animation.style.display="flex"
            container.style.display="none"
            setTimeout(()=>{window.location.href="steak_remember.html"},1200)
        }
        catch(error){
            alert("嗚嗚嗚，錯了")
            location.reload()
