...
const docRef=doc(db,"forms","beefnoodles")
let ifShow
//loading
...
if(ifShow.show==true){
    ...
    const zoneArea=document.getElementById("zone")
    const smellArea=document.getElementById("smell")
    const zone=document.getElementById("exRice1")
    const smell=document.getElementById("exRice2")
    ...
    const re =()=>{
        ...
        smell.style.display="none"
        smellArea.style.display="none"
        smell.checked=false
        zone.checked=false
    }
    const allType_1=document.querySelectorAll('.type_1')
    const allType_2=document.querySelectorAll('.type_2')
    allType_1.forEach((element)=>{
        element.addEventListener("change",()=>{
        re()
        zoneArea.style.display="block"
        zone.style.display="block"
        document.getElementById("s").style.display="none"
        zoneArea.scrollIntoView({behavior:"smooth"})
        })
    })
    allType_2.forEach((element)=>{
        element.addEventListener("change",()=>{
        re()
        zoneArea.style.display="block"
        zone.style.display="block"
        smellArea.style.display="block"
        smell.style.display="block"
        document.getElementById("s").style.display="block"
        smellArea.scrollIntoView({behavior:"smooth"})
        })
    })
    ...
    const handleSubmit=async(e)=>{
        ...
        meal=meal.value
        if(zone.checked==true&&smell.checked==true){
            meal=meal+"+蔥"+"+香菜"
        }
        else if(zone.checked==true){
            meal=meal+"+蔥"
        }
        else if(smell.checked==true){
            meal=meal+"+香菜"
        }
        //準備傳送(建立物件)
        const total={
        ...
        try{
            ...
            setTimeout(()=>{window.location.href="beefnoodles_remember.html"},1200)
        }
        catch(error){
            alert("嗚嗚嗚，錯了")
            location.reload()
