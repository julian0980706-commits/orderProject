...
let myNumber = localStorage.getItem("steak");
...
const orderRef_bytime=query(orderRef,
    ...
    let ifShow
    try{
        ifShow= await getDoc(doc(db,"forms","steak"))
    }
    catch(e){
    ...
    order_array.forEach((element)=>{
        ...
        if(mealcounter[meal.replace(/\d/g, "")]){
            if(mealcounter[meal.replace(/\d/g, "")][sauce]){
                mealcounter[meal.replace(/\d/g, "")][sauce]+=1
            }
            else{
                mealcounter[meal.replace(/\d/g, "")][sauce]=1
            }
