...
const docRef=doc(db,"forms","firemeat")
let ifShow
//loading
...
if(ifShow.show==true){
    //checkbox邏輯
    const exRice1= document.getElementById("exRice1")
    const exRice2= document.getElementById("exRice2")
    exRice1.addEventListener("change",()=>{
        exRice2.checked=false
    })
    exRice2.addEventListener("change",()=>{
        exRice1.checked=false
    })
    ...
    //
    const orderRef=collection(db,"forms","firemeat","firemeat",)
    ...
    const handleSubmit=async(e)=>{
        ...
        const checkboxs=document.getElementsByName("extra")
        const checkeds=[]
        ...
        try{
            ...
            //傳完才執行下面
            localStorage.setItem("firemeat", num);
            submit_btn.innerText="送出訂單"
            main.style.display="none"
            animation.style.display="flex"
            container.style.display="none"
            setTimeout(()=>{
                window.location.href="firemeat_remember.html"
            },1200)
