...
const docRef=doc(db,"forms","lunchbox")
let ifShow
//loading
...
if(ifShow.show==true){
    ...
    //
    const orderRef=collection(db,"forms","lunchbox","lunchbox",)
    ...
    const handleSubmit=async(e)=>{
        ...
        try{
            ...
            //傳完才執行下面
            localStorage.setItem("lunchbox", num);
            submit_btn.innerText="送出訂單"
            main.style.display="none"
            animation.style.display="flex"
            animation.style.display="flex"
            container.style.display="none"
            setTimeout(()=>{window.location.href="lunchbox_remember.html"},1200)
        }
        catch(error){
            alert("嗚嗚嗚，錯了")
            location.reload()
