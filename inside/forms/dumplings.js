...
const docRef=doc(db,"forms","dumplings")
let ifShow
//loading
...
if(ifShow.show==true){
    ...
    const num_area=document.getElementById("num")
    num_area.addEventListener("input",error_deal)
    ...
    const handleSubmit=async(e)=>{
        ...
        const howMany=document.querySelector('#num').value
        ...
        if(extra&&sidedish=="鍋貼或水餃"&&!meal&&!howMany){
            total={
            num:num,
            meal:"",
            sidedish:"",
            ...
            if(!howMany){
                error_deal()
                eDnum.style.display="block"
                eDnum.scrollIntoView({behavior:"smooth"})//滾動到那裏(smoothly)
                return;
            }
            total={
            num:num,
        ...
        try{
            ...
            //傳完才執行下面
            localStorage.setItem("dumplings", num);
            submit_btn.innerText="送出訂單"
            main.style.display="none"
            animation.style.display="flex"
            container.style.display="none"
            setTimeout(()=>{window.location.href="dumplings_remember.html"},1200)
        }
        catch(error){
            alert("嗚嗚嗚，錯了")
            location.reload()
